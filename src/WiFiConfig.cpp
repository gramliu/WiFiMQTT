...
void WiFiConfig::save_config(bool readFromPortal) {
    if (readFromPortal) {
...
void WiFiConfig::on_msg_received(const char* topic, byte* payload,
    ...
    if (err) {
        ...
        if (obj.containsKey("topic")) {
            const char* newTopic = obj["topic"];
            strncpy(WiFiConfig::tmp_topic, newTopic, strlen(newTopic));
            WiFiConfig::set_new_topic = true;
            Serial.print("Changing MQTT topic to: ");
            Serial.println(WiFiConfig::tmp_topic);
        } else {
            Serial.println("Resetting config");
            WiFiConfig::do_reset_config = true;
        }
...
void WiFiConfig::check_connection() {
    ...
    if (WiFiConfig::set_new_topic) {
        strncpy(mqtt_config->mqtt_topic, WiFiConfig::tmp_topic,
                strlen(WiFiConfig::tmp_topic));
    }
    if (WiFiConfig::do_save_config) {
        save_config(!WiFiConfig::set_new_topic);
        WiFiConfig::do_save_config = false;
    }
    WiFiConfig::set_new_topic = false;
    if (!client.connected()) {
        setup_mqtt();
        setup_config();
    }
...
boolean WiFiConfig::set_new_topic = false;
char WiFiConfig::tmp_topic[MQTT_Topic_Size];
